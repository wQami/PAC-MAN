...
namespace Game {
VictoryState::VictoryState(const shared_ptr<StateManager>& context) : State(context) {}
...
void VictoryState::draw(RenderWindow& window) {
    ...
    highscoresHeader.setString("VICTORY!");
    highscoresHeader.setOrigin(120.5, 15.5);
...
void VictoryState::keyPress() {
    if (Keyboard::isKeyPressed(sf::Keyboard::Space)) {
        getStateManager()->incLevel();
...
void VictoryState::handlePeriod() {}
} // namespace Game